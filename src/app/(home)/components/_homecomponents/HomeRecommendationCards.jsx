...
/** How many cards show at once; keep in sync with CSS in newmpfmetadata.css (spacing only). */
function getVisibleCount(viewportWidth, itemCount) {
  let v;
  if (viewportWidth <= 576) v = 1;
  else if (viewportWidth <= 768) v = 2;
  else if (viewportWidth <= 1200) v = 3;
  else v = 4;
  return Math.min(v, Math.max(1, itemCount));
}
...
export default function HomeRecommendationCards({
  ...
  /** Track is (n/v)× viewport width; each slide is 1/n of track so each occupies 100/v % of viewport. */
  const trackStyle = useMemo(() => {
    if (!safeItems.length || !visibleCount) return {};
    const trackWidthPct = (safeItems.length * 100) / visibleCount;
    const shiftPct = (startIndex * visibleCount * 100) / safeItems.length;
    return {
      width: `${trackWidthPct}%`,
      transform: `translateX(-${shiftPct}%)`,
    };
  }, [safeItems.length, visibleCount, startIndex]);

  const slideStyle = useMemo(
    () => ({
      flex: `0 0 ${safeItems.length ? 100 / safeItems.length : 100}%`,
      minWidth: 0,
    }),
    [safeItems.length],
  );
  ...
  useEffect(() => {
    const updateVisibleCount = () => {
      setVisibleCount(
        getVisibleCount(window.innerWidth, safeItems.length),
      );
    };
  ...
  return (
    <section
      className={`home-projects-preview ${className}`.trim()}
      aria-label={title}
    >
        ...
        <div className="home-projects-preview__track" style={trackStyle}>
          {safeItems.map((item, idx) => {
            ...
            return (
              <div
                key={rowKey}
                className="home-projects-preview__slide"
                style={slideStyle}
              >
                <Link href={card.href} className="home-project-card">
                  <div className="home-project-card__media">
                    <Image
                      ...
                      fill
                      sizes="(max-width: 576px) 92vw, (max-width: 768px) 46vw, (max-width: 1200px) 34vw, 26vw"
